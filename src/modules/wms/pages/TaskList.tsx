...
import type { TaskFilters } from '@/services/wms.service';
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[rgb(var(--border)/0.4)]">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={9} className="px-4 py-12 text-center text-sm text-[rgb(var(--text-muted))]">
                    {t('task.noResult')}
                  </td>
                </tr>
              ) : (
                filtered.map((task) => {
                  ...
                  return (
                    <tr key={task._id} className="hover:bg-[rgb(var(--bg-hover))] transition-colors">
                      ...
                      <td className="px-4 py-2.5">
                        <div>
                          ...
                          {task.projectName && (
                            <Badge variant="neutral" size="sm" className="mt-1">{task.projectName}</Badge>
                          )}
                        </div>
                      </td>
                      <td className="px-4 py-2.5 text-[rgb(var(--text-secondary))]">
                        <div className="flex items-center gap-2">
                          <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[rgb(var(--primary)/0.1)] text-[10px] font-bold text-[rgb(var(--primary))]">
                            {(task.assignedToName ?? task.assignedTo ?? '?').split(' ').map((n: string) => n[0]).slice(-2).join('')}
                          </div>
                          <span className="text-xs">{task.assignedToName ?? task.assignedTo ?? '-'}</span>
                        </div>
                      </td>
                      <td className="px-4 py-2.5 text-xs text-[rgb(var(--text-muted))]">{task.projectName ?? '-'}</td>
                      <td className="px-4 py-2.5">
                      ...
                      <td className="px-4 py-2.5 text-xs tabular-nums text-[rgb(var(--text-muted))]">
                        {isOverdue ? (
                          <span className="text-[rgb(var(--error))] font-medium">{t('task.overdue')}: {task.dueDate}</span>
                        ) : (
                          task.dueDate ?? '-'
                        )}
                      </td>
                      ...
                      <td className="px-4 py-2.5">
                        <Button variant="ghost" size="sm">
                          <Link to={`/wms/cong-viec/${task._id}`}>
