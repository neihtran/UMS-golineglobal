...
import {
  ...
  useUpdateHqnhatStudentReservation,
  useDeleteHqnhatStudentReservation,
  useHqnhatStudents,
  useHqnhatMajors,
...
import type {
  HqnhatStudent,
  HqnhatStudentReservation,
  HqnhatStudentReservationCreatePayload,
  HqnhatAcademicTerm,
...
const emptyForm = (): HqnhatStudentReservationCreatePayload => ({
  student_id: 0,
  from_date: '',
  to_date: '',
  semester_from_id: undefined,
  semester_to_id: undefined,
  decision_no: '',
  decision_date: '',
  reason: '',
  status: 1,
...
export function BaoLuuSheet() {
  ...
  const { data, isLoading, isFetching } = useHqnhatStudentReservations(params);
  const { data: studentData } = useHqnhatStudents({ per_page: 100 });
  const { data: majorData } = useHqnhatMajors({ per_page: 100, status: 1 });
  const { data: termData } = useHqnhatAcademicTerms({ per_page: 100 });
  ...
  const semesters = Array.isArray(termData?.data) ? termData.data : [];

  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<HqnhatStudentReservation | null>(null);
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleting, setDeleting] = useState<HqnhatStudentReservation | null>(null);
  const [detailOpen, setDetailOpen] = useState(false);
  const [detailItem, setDetailItem] = useState<HqnhatStudentReservation | null>(null);
  const [form, setForm] = useState<HqnhatStudentReservationCreatePayload>(emptyForm());
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [submitError, setSubmitError] = useState<string | null>(null);

  const createMut = useCreateHqnhatStudentReservation();
  const updateMut = useUpdateHqnhatStudentReservation();
  const deleteMut = useDeleteHqnhatStudentReservation();
  ...
  const getSemesterName = (id: number | string | null) => {
    if (!id) return '—';
    const numId = typeof id === 'string' ? Number(id) : id;
    const s = semesters.find((t: HqnhatAcademicTerm) => Number(t.id) === numId);
    return s?.code ?? `HK #${id}`;
  };
  ...
  const openEdit = (item: HqnhatStudentReservation) => {
    setEditing(item);
    setForm({
      student_id: item.student_id,
      from_date: normalizeDate(item.from_date),
      to_date: normalizeDate(item.to_date),
      semester_from_id: item.semester_from_id ?? undefined,
      semester_to_id: item.semester_to_id ?? undefined,
  ...
  const openDetail = (item: HqnhatStudentReservation) => {
    setDetailItem(item);
    setDetailOpen(true);
  };

  const openDelete = (item: HqnhatStudentReservation) => {
    setDeleting(item);
    setDeleteOpen(true);
  };
  ...
  const validate = (): boolean => {
    ...
    if (!form.from_date) e.from_date = 'Chọn ngày bắt đầu bảo lưu';
    if (!form.to_date) e.to_date = 'Chọn ngày kết thúc bảo lưu';
    if (form.from_date && form.to_date && form.from_date >= form.to_date) {
      e.to_date = 'Ngày kết thúc phải sau ngày bắt đầu';
    }
    setErrors(e);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        semester_from_id: form.semester_from_id ? Number(form.semester_from_id) : undefined,
        semester_to_id: form.semester_to_id ? Number(form.semester_to_id) : undefined,
      };
      if (editing) {
        ...
        // Cập nhật trạng thái sinh viên thành Bảo lưu (2)
  ...
  return (
    <div className="space-y-4">
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableHeadCell>HK bắt đầu</TableHeadCell>
            <TableHeadCell>HK kết thúc</TableHeadCell>
          ...
          {isLoading ? (
            ...
            <TableRow>
              <TableCell colSpan={10} className="text-center py-10 text-[rgb(var(--text-muted))]">
                {search || statusFilter ? 'Không tìm thấy kết quả phù hợp' : 'Chưa có bản ghi bảo lưu nào'}
              </TableCell>
            </TableRow>
          ) : (
            items.map((item: HqnhatStudentReservation, i) => {
              ...
              return (
                <TableRow key={item.id} className={isFetching && !isLoading ? 'opacity-50' : ''}>
                  ...
                  <TableCell className="text-sm">{formatDate(item.from_date)}</TableCell>
                  <TableCell className="text-sm">{formatDate(item.to_date)}</TableCell>
                  <TableCell className="text-sm">{getSemesterName(item.semester_from_id)}</TableCell>
                  <TableCell className="text-sm">{getSemesterName(item.semester_to_id)}</TableCell>
                  <TableCell>
      ...
      <Modal
        ...
        title={editing ? 'Sửa bảo lưu' : 'Tạo bảo lưu sinh viên'}
        size="lg"
        footer={
          <>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Từ ngày" error={errors.from_date} required>
              <input type="date" value={form.from_date} onChange={(e) => setForm({ ...form, from_date: e.target.value })} className="h-10 w-full rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--bg-card))] px-3 text-sm" />
            </FormField>
            <FormField label="Đến ngày" error={errors.to_date} required>
              <input type="date" value={form.to_date} onChange={(e) => setForm({ ...form, to_date: e.target.value })} className="h-10 w-full rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--bg-card))] px-3 text-sm" />
            </FormField>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Học kỳ bắt đầu">
              <select value={form.semester_from_id ?? ''} onChange={(e) => setForm({ ...form, semester_from_id: e.target.value ? Number(e.target.value) : undefined })} className="h-10 w-full rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--bg-card))] px-3 text-sm">
                <option value="">— Chọn —</option>
                {semesters.map((s: HqnhatAcademicTerm) => <option key={s.id} value={s.id}>{s.code}</option>)}
              </select>
            </FormField>
            <FormField label="Học kỳ kết thúc">
              <select value={form.semester_to_id ?? ''} onChange={(e) => setForm({ ...form, semester_to_id: e.target.value ? Number(e.target.value) : undefined })} className="h-10 w-full rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--bg-card))] px-3 text-sm">
                <option value="">— Chọn —</option>
                {semesters.map((s: HqnhatAcademicTerm) => <option key={s.id} value={s.id}>{s.code}</option>)}
              </select>
            </FormField>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Số quyết định">
              <input type="text" value={form.decision_no ?? ''} onChange={(e) => setForm({ ...form, decision_no: e.target.value })} placeholder="VD: QD-123/2024" className="h-10 w-full rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--bg-card))] px-3 text-sm" />
            </FormField>
          ...
          <FormField label="Lý do">
            <textarea value={form.reason ?? ''} onChange={(e) => setForm({ ...form, reason: e.target.value })} placeholder="Nhập lý do bảo lưu..." rows={3} className="w-full rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--bg-card))] px-3 py-2 text-sm resize-none" />
          </FormField>
        </div>
      </Modal>

      <Modal open={detailOpen} onClose={() => setDetailOpen(false)} title="Chi tiết bảo lưu" size="lg">
        {detailItem ? (
          <div className="space-y-3 text-sm">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><span className="text-[rgb(var(--text-muted))]">Từ ngày:</span> <span className="ml-1">{formatDate(detailItem.from_date)}</span></div>
              <div><span className="text-[rgb(var(--text-muted))]">Đến ngày:</span> <span className="ml-1">{formatDate(detailItem.to_date)}</span></div>
              <div><span className="text-[rgb(var(--text-muted))]">HK bắt đầu:</span> <span className="ml-1">{getSemesterName(detailItem.semester_from_id)}</span></div>
              <div><span className="text-[rgb(var(--text-muted))]">HK kết thúc:</span> <span className="ml-1">{getSemesterName(detailItem.semester_to_id)}</span></div>
      ...
      <ConfirmModal
        ...
        title="Xóa bản ghi bảo lưu"
        description={`Xác nhận xóa bản ghi bảo lưu của sinh viên "${deleting ? getStudentName(deleting.student_id) : ''}"?`}
        confirmText="Xóa"
        variant="danger"
