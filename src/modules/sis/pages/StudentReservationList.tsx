...
const RESERVATION_STATUS: Record<number, { label: string; variant: 'success' | 'warning' | 'error' | 'neutral' }> = {
...
export default function StudentReservationList() {
  ...
  const { data, isLoading, isFetching, refetch } = useHqnhatStudentReservations(params);
  ...
  const getSemesterName = (id: number | string | null) => {
    ...
    const numId = typeof id === 'string' ? Number(id) : id;
    const s = semesters.find((s: any) => Number(s.id) === numId);
    return s?.code ?? `HK #${id}`;
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <Table>
        ...
        <TableBody>
          {isLoading ? (
            <TableSkeleton rows={5} colSpan={11} />
          ) : items.length === 0 ? (
            <TableRow>
              <TableCell colSpan={11} className="text-center py-12 text-[rgb(var(--text-muted))]">
            ...
            items.map((item: HqnhatStudentReservation, i) => {
              ...
              return (
                <TableRow key={item.id} className={isFetching && !isLoading ? 'opacity-50' : ''}>
                  ...
                  <TableCell>
                    <Badge variant={RESERVATION_STATUS[item.status]?.variant ?? 'neutral'} dot size="sm">
                      {RESERVATION_STATUS[item.status]?.label ?? `Trạng thái ${item.status}`}
                    </Badge>
                  </TableCell>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Học kỳ bắt đầu">
              <select
                value={form.semester_from_id ?? ''}
                onChange={(e) => setForm({ ...form, semester_from_id: e.target.value ? Number(e.target.value) : undefined })}
                className="h-10 w-full rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--bg-card))] px-3 text-sm"
              >
                <option value="">— Chọn —</option>
                {semesters.map((s: any) => <option key={s.id} value={s.id}>{s.code}</option>)}
              </select>
            </FormField>
            <FormField label="Học kỳ kết thúc">
              <select
                value={form.semester_to_id ?? ''}
                onChange={(e) => setForm({ ...form, semester_to_id: e.target.value ? Number(e.target.value) : undefined })}
                className="h-10 w-full rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--bg-card))] px-3 text-sm"
              >
                <option value="">— Chọn —</option>
                {semesters.map((s: any) => <option key={s.id} value={s.id}>{s.code}</option>)}
