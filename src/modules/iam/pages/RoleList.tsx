...
const ROLES = [
  ...
  { id: 'r07', code: 'NHAN_VIEN', name: 'Nhân viên', description: 'Truy cập HRM, DMS, FIN, WMS cấp phòng ban', userCount: 89, permissions: 32, status: 'active', createdAt: '2024-02-10' },
];
...
const DEFAULT_PERMISSIONS: PermMatrix = {
  ...
  GIAO_VIEN: Object.fromEntries(['SIS', 'LMS', 'EXAM', 'DMS', 'WMS', 'PORTAL', 'LIB'].map(m => [m, { read: true, write: true, approve: false }])),
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        ...
        description="IAM-01 — Ma trận quyền module × hành động cho 7 vai trò hệ thống"
            ...
            {paged.map((r) => (
              ...
              >
                <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${
                  r.code === 'SUPER_ADMIN' ? 'bg-[rgb(var(--primary)/0.1)] text-[rgb(var(--primary))]' :
                  r.code === 'SINH_VIEN' ? 'bg-[rgb(var(--info)/0.1)] text-[rgb(var(--info))]' :
                  r.code === 'GIAO_VIEN' ? 'bg-[rgb(var(--accent)/0.1)] text-[rgb(var(--accent))]' :
