...
import { Plus, Search, Edit, Trash2, Eye, RotateCcw, Briefcase } from 'lucide-react';
import {
  Button,
  Input,
  Badge,
...
const emptyForm = (): PositionCreatePayload => ({
  code: '',
  name: '',
  description: null,
  sort_order: 0,
...
export function PositionSheet() {
  ...
  const { data, isLoading, isFetching } = usePositions(params);
  ...
  const [form, setForm] = useState<PositionCreatePayload>(emptyForm());
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [submitError, setSubmitError] = useState<string | null>(null);

  const { data: detailData, isLoading: detailLoading } = usePosition(detailId ?? undefined);
  ...
  const openDetail = (item: Position) => {
    setDetailId(item.id);
    setDetailOpen(true);
  };

  const openDelete = (item: Position) => {
    setDeleting(item);
    setDeleteOpen(true);
  };
  ...
  const validate = (): boolean => {
    ...
    if (!form.code.trim()) e.code = 'Mã chức vụ không được để trống';
    if (!form.name.trim()) e.name = 'Tên chức vụ không được để trống';
    setErrors(e);
    ...
    <div className="space-y-4">
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableHeadCell>Tên chức vụ</TableHeadCell>
              ...
              <TableRow key={item.id} className={isFetching && !isLoading ? 'opacity-50' : ''}>
                ...
                <TableCell className="font-medium">
                  <div className="flex items-center gap-2">
                    <Briefcase className="h-3.5 w-3.5 text-[rgb(var(--text-muted))]" />
                    {item.name}
                  </div>
                </TableCell>
      ...
      >
        <div className="space-y-4">
          ...
          <FormField label="Mã chức vụ" error={errors.code} required>
            <Input
              value={form.code}
              onChange={(e) => setForm({ ...form, code: e.target.value })}
              placeholder="VD: TP, PP, TK"
            />
          </FormField>
          <FormField label="Tên chức vụ" error={errors.name} required>
            <Input
              value={form.name}
      ...
      <ConfirmModal
        ...
        description={`Bạn có chắc muốn xóa chức vụ "${deleting?.name}"?`}
        confirmText="Xóa"
        variant="danger"
        loading={deleteMut.isPending}
      />

      <Modal open={detailOpen} onClose={() => setDetailOpen(false)} title="Chi tiết chức vụ" size="sm">
        {detailLoading ? (
