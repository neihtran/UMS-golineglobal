...
import { Plus, Search, Edit, Trash2, Eye, RotateCcw, Presentation } from 'lucide-react';
import {
  Button,
  Input,
  Badge,
...
import { useHqnhatCourseSections } from '@/hooks/useHqnhat';
import type {
  TeachingAssignment,
  TeachingAssignmentCreatePayload,
  EmployeeProfile,
} from '@/types/hrm.types';
import type { HqnhatCourseSection } from '@/types/hqnhat.types';

const emptyForm = (): TeachingAssignmentCreatePayload => ({
  lecturer_id: 0,
...
export function TeachingAssignmentSheet() {
  ...
  const { data, isLoading, isFetching } = useTeachingAssignments(params);
  const { data: employeesData } = useEmployeeProfiles({ per_page: 100 });
  const { data: courseSectionsData } = useHqnhatCourseSections({ per_page: 100 });

  const items = Array.isArray(data?.data) ? data.data : [];
  const total = data?.meta?.total ?? items.length;
  const employees = Array.isArray(employeesData?.data) ? employeesData.data : [];
  const courseSections = Array.isArray(courseSectionsData?.data) ? courseSectionsData.data : [];

  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<TeachingAssignment | null>(null);
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleting, setDeleting] = useState<TeachingAssignment | null>(null);
  const [detailOpen, setDetailOpen] = useState(false);
  const [detailId, setDetailId] = useState<number | null>(null);
  const [form, setForm] = useState<TeachingAssignmentCreatePayload>(emptyForm());
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [submitError, setSubmitError] = useState<string | null>(null);

  const { data: detailData, isLoading: detailLoading } = useTeachingAssignment(detailId ?? undefined);
  const createMut = useCreateTeachingAssignment();
  const updateMut = useUpdateTeachingAssignment();
  const deleteMut = useDeleteTeachingAssignment();
  ...
  const getStatusConfig = (status: string) => {
    const configs: Record<string, { variant: 'success' | 'warning' | 'error' | 'neutral' | 'info'; label: string }> = {
  ...
  const openDetail = (item: TeachingAssignment) => {
    setDetailId(item.id);
    setDetailOpen(true);
  };

  const openDelete = (item: TeachingAssignment) => {
    setDeleting(item);
    setDeleteOpen(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError(null);
    try {
      const payload: TeachingAssignmentCreatePayload = { ...form };
      editing
  ...
  return (
    <div className="space-y-4">
                ...
                <TableRow key={item.id} className={isFetching && !isLoading ? 'opacity-50' : ''}>
                  ...
                  <TableCell className="font-medium">
                    <div className="flex items-center gap-2">
                      <Presentation className="h-3.5 w-3.5 text-[rgb(var(--text-muted))]" />
                      {getLecturerName(item.lecturer_id)}
                    </div>
                  </TableCell>
                    <TableCell className="text-sm">
                      {(TEACHING_TYPE_OPTIONS.find(o => o.value === item.teaching_type)?.label) ?? item.teaching_type ?? '—'}
                    </TableCell>
                  <TableCell className="text-sm tabular-nums">{item.credit || '—'}</TableCell>
                  <TableCell className="text-sm tabular-nums">{item.teaching_hours || '—'}</TableCell>
      ...
      <Modal
        ...
        title={editing ? 'Sửa phân công GD' : 'Thêm phân công GD'}
        size="md"
        footer={
          <>
        ...
        <div className="space-y-4">
          ...
          <FormField label="Lớp/Phần học">
            ...
            >
              ...
              {courseSections.map((cs: HqnhatCourseSection) => (
          ...
          <div className="grid grid-cols-3 gap-4">
            <FormField label="Loại giảng dạy">
              <select
                value={form.teaching_type ?? ''}
                onChange={(e) => setForm({ ...form, teaching_type: e.target.value || null })}
                className="h-10 w-full rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--bg-card))] px-3 text-sm"
              >
            ...
            <FormField label="Tín chỉ">
              <Input
                type="number"
                value={form.credit ?? ''}
                onChange={(e) => setForm({ ...form, credit: e.target.value ? Number(e.target.value) : null })}
              />
            </FormField>
            <FormField label="Số giờ">
              <Input
                ...
                onChange={(e) => setForm({ ...form, teaching_hours: e.target.value ? Number(e.target.value) : null })}
              />
            </FormField>
          </div>
      ...
      <Modal open={detailOpen} onClose={() => setDetailOpen(false)} title="Chi tiết phân công GD" size="md">
        {detailLoading ? (
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 pb-2 border-b">
              <Presentation className="h-5 w-5 text-[rgb(var(--primary))]" />
              <h3 className="text-lg font-bold">Phân công giảng dạy</h3>
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-[rgb(var(--bg-secondary))] rounded-lg p-3">
                <p className="text-xs text-[rgb(var(--text-muted))] mb-1">Loại</p>
                <p className="font-medium">
                  {TEACHING_TYPE_OPTIONS.find(o => o.value === detailData.data.teaching_type)?.label ?? '—'}
                </p>
              </div>
              <div className="bg-[rgb(var(--bg-secondary))] rounded-lg p-3">
                <p className="text-xs text-[rgb(var(--text-muted))] mb-1">Tín chỉ</p>
                <p className="font-medium">{detailData.data.credit || '—'}</p>
              </div>
              <div className="bg-[rgb(var(--bg-secondary))] rounded-lg p-3">
                <p className="text-xs text-[rgb(var(--text-muted))] mb-1">Số giờ</p>
                <p className="font-medium">{detailData.data.teaching_hours || '—'}</p>
              </div>
              <div className="bg-[rgb(var(--bg-secondary))] rounded-lg p-3">
...
export default TeachingAssignmentSheet;
